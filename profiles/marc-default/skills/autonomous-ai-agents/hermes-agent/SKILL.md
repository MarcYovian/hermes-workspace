...
**Important:** All cron schedules are interpreted in **UTC**, not local time. For users in non-UTC timezones (e.g., WIB = UTC+7), you must convert local time to UTC when creating schedules. See `skill_view(name="hermes-agent", file_path="references/cron-timezone-wib.md")` for conversion examples and cross-profile cron editing patterns.
...
| Profile switching | **NO — same profile only** | **YES — can use -p flag** |
| Use case | Quick parallel subtasks within same profile | Long autonomous missions OR switching to specialist profiles |

**CRITICAL:** `delegate_task` cannot spawn a different Hermes profile. It creates a sub-agent within the **same profile** with isolated context. To delegate work to another profile (e.g., orchestrator → specialist), you MUST spawn an independent Hermes process via `terminal()` with the `-p <profile>` flag.
