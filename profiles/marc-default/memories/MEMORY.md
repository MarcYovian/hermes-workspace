...
§
User runs Hermes in Docker container. Hermes binary location: /opt/hermes/bin/hermes. Profiles directory: /opt/data/profiles/ (contains marc-default, dev-coder, devops-admin).
§
Hermes environment:
- Hermes executable: /opt/hermes/bin/hermes
- Active profiles: marc-default (current), dev-coder, devops-admin
- Profile data: /opt/data/profiles/
- Hermes CLI tidak ada di PATH, harus pakai full path
§
VPS Sinyora (103.196.154.123): SSH key di /opt/data/profiles/devops-admin/ssh/sinyora-vps.pem, akses via `ssh sinyora-vps` (SSH config sudah setup). Task yang butuh akses VPS → delegasikan ke Aegis.
§
...
§
Profile dev-coder diberi nama "Kairo" oleh user. Gateway Telegram disabled (komunikasi via SOUL). Rule wajib: semua project harus berjalan di atas Docker.
§
